...
from model.siamese_loss.triplet import TripletLoss
...
if __name__ == "__main__":
    ...
    criterion = TripletLoss(m=opt.distance_threshold)
    ...
    train_dataset = TripletDataset(opt.img_dir, data_df, opt)
    ...
    for epoch in pbar:
        ...
        for X, y in train_loader:
            ...
            X = X.to(device)
            y = y.to(device)
            embedd = model.single_forward(X)
            loss = criterion(embedd, y)
            loss.backward()
            optimizer.step()
